...
import { Matcher, type MatchOutcome, type MatchResult } from "./matcher/matcher";
import { matchSmart } from "./matchStrategy";
import { detectWidgetQuad, rectifyQuad, tryRectifyWidget } from "./camera/rectify";
...
const resultMainEl = document.querySelector<HTMLDivElement>("#result-main")!;
const resultCopyHintEl = document.querySelector<HTMLParagraphElement>("#result-copy-hint")!;
const fileInputEl = document.querySelector<HTMLInputElement>("#file-input")!;
const fileStatusEl = document.querySelector<HTMLSpanElement>("#file-status")!;
...
/** 結果パネルに現在表示中のエントリ。「タップして座標をコピー」で使う。 */
let lastShownEntry: MapEntry | null = null;
...
function zoneDisplayName(entry: MapEntry): string {
...
/** 選択言語での地図アイテム表示名。 */
function gradeDisplayName(entry: MapEntry): string {
  return db?.gradeNames?.[entry.grade]?.[lang] ?? entry.gradeName;
}

/** 座標表記はゲーム内の <pos> 形式で全言語共通。 */
function locationText(entry: MapEntry): string {
  return `${zoneDisplayName(entry)} (X:${entry.x.toFixed(1)}, Y:${entry.y.toFixed(1)})`;
}
...
async function loadDatabase(): Promise<void> {
  ...
  try {
    ...
    // 地名が無いエントリ）は照合・候補・OCRリストのすべてから除外する。
    ...
    zoneKeyByName = new Map();
    zoneNameByKey = new Map();
    for (const e of entries) {
      const key = zoneKeyOf(e);
      const name = zoneDisplayName(e);
      zoneKeyByName.set(name, key);
      zoneNameByKey.set(key, name);
    }
    ocrZoneNames = [...zoneKeyByName.keys()];
    hideLoading();
    syncLoopRunning();
  } catch (err) {
    matcher = null;
...
// --- 静止画像からの判定（PCスクショの選択・貼り付け・ドロップ） -------------------
// Win+Shift+S 等で撮ったスクリーンショットのように、既に地図ウィンドウ付近を
// 切り取り済みの静止画を 1 枚だけ照合する。ライブスキャンと同じ照合エンジン
// （detectWidgetQuad → rectifyQuad → matchSmart）をそのまま流用する。

/** 静止画像はカメラのような枚数制約がないため、この幅までに抑えて処理する。 */
const STATIC_IMAGE_MAX_WIDTH = 1600;

/** 画像ソースから、照合エンジンに渡せる GrayImage を作る。 */
async function decodeToGrayImage(source: Blob): Promise<GrayImage> {
  const bitmap = await createImageBitmap(source);
  try {
    const scale = bitmap.width > STATIC_IMAGE_MAX_WIDTH ? STATIC_IMAGE_MAX_WIDTH / bitmap.width : 1;
    ...
    return GrayImage.fromImageData(ctx.getImageData(0, 0, w, h));
  } finally {
    bitmap.close();
  }
}

/** 静止画 1 枚の照合結果を結果パネル・候補パネルに反映する。 */
function presentStaticMatch(outcome: MatchOutcome): void {
  hideCandidates();
  const best = outcome.best;
  if (!best) return;
  if (outcome.isConfident) {
    renderResultPanel(best.entry, Math.round(outcome.confidence * 100), true);
    navigator.vibrate?.(100);
  } else {
    // 1 枚だけの静止画照合はライブスキャンのような複数フレームでの収束が効かないため、
    // 不確実な場合は暫定のトップ候補を表示しつつ、他の候補もタップで選べるようにする。
    renderResultPanel(best.entry, matchDegreePct(best.ncc), false);
    showCandidates(outcome.candidates);
  }
}

/** 静止画像 1 枚を解析し、結果パネルに反映する。カメラの有無に関わらず利用できる。 */
async function analyzeStaticImage(source: Blob): Promise<void> {
  if (!matcher) {
    fileStatusEl.textContent = msg("dbLoading");
    return;
  }
  hideResultPanel();
  hideCandidates();
  wantScanning = false;
  syncLoopRunning();
  fileStatusEl.textContent = msg("analyzingImage");
  try {
    const grayFull = await decodeToGrayImage(source);
    const rectified = tryRectifyWidget(grayFull);
    lastRectifiedForDisplay = rectified ?? grayFull;
    const outcome = matchSmart(matcher, grayFull, { rectified, zoneKey: null });
    presentStaticMatch(outcome);
    fileStatusEl.textContent = "";
  } catch (err) {
    console.warn("静止画像の解析でエラーが発生しました:", err);
    fileStatusEl.textContent = msg("staticAnalyzeFailed");
  }
}

fileInputEl.addEventListener("change", () => {
  const file = fileInputEl.files?.[0];
  // 同じファイルを連続で選び直せるよう、読み取り後すぐに値をリセットする。
  fileInputEl.value = "";
  if (file) void analyzeStaticImage(file);
});

// クリップボードへの画像貼り付け（Win+Shift+S 等でコピーした画像を Ctrl+V で直接判定）。
...
// 画面へのドラッグ＆ドロップにも対応する（エクスプローラ上の画像ファイルなど）。
viewfinderEl.addEventListener("dragover", (ev) => {
  ev.preventDefault();
});
viewfinderEl.addEventListener("drop", (ev) => {
  ev.preventDefault();
  const file = ev.dataTransfer?.files?.[0];
  if (file && file.type.startsWith("image/")) void analyzeStaticImage(file);
});
...
function renderResultPanel(entry: MapEntry, confidencePct: number, confident: boolean): void {
  ...
  resultZoneEl.textContent = locationText(entry);
  resultGradeEl.textContent = gradeDisplayName(entry);
...
/** コピー結果のフィードバック表示を既定の案内文言に戻す。 */
function resetCopyHint(): void {
  if (copyHintResetTimer !== null) {
    window.clearTimeout(copyHintResetTimer);
    copyHintResetTimer = null;
  }
  resultCopyHintEl.textContent = msg("copyHint");
  resultCopyHintEl.classList.remove("copy-success");
}

/** 現在表示中の座標を「エリア名 (X:xx.x, Y:yy.y)」形式でクリップボードにコピーする。 */
async function copyShownLocation(): Promise<void> {
  if (!lastShownEntry) return;
  const text = locationText(lastShownEntry);
  try {
    await navigator.clipboard.writeText(text);
    resultCopyHintEl.textContent = msg("copied");
    resultCopyHintEl.classList.add("copy-success");
  } catch (err) {
    console.warn("クリップボードへのコピーに失敗しました:", err);
    resultCopyHintEl.textContent = msg("copyFailed");
    resultCopyHintEl.classList.remove("copy-success");
  }
  if (copyHintResetTimer !== null) window.clearTimeout(copyHintResetTimer);
  copyHintResetTimer = window.setTimeout(resetCopyHint, 1600);
}

resultMainEl.addEventListener("click", () => {
  void copyShownLocation();
});
resultMainEl.addEventListener("keydown", (ev) => {
...
function matchDegreePct(ncc: number): number {
...
function showCandidates(candidates: readonly MatchResult[]): void {
  ...
  for (const c of candidates.slice(0, 3)) {
    ...
    li.textContent = `${locationText(c.entry)} ・${msg("matchDegree", { pct: matchDegreePct(c.ncc) })}`;
    li.tabIndex = 0;
